...
async def get_horario_audiencia(
    ...
    mayor audiencia". El frontend agrupa por día de semana × hora; acá se
    filtra por programa exacto o por canal (exactamente uno, validado en el
    service), opcionalmente acotado además por `tipo` (podcast/programa,
    mismo criterio que kpis/evolutivo/ranking — relevante sobre todo en
    modo canal, donde puede haber programas de ambos tipos), y por rango de
    fechas, sin agregar (el grano ya es diario, 1 fila = 1 día en el rango
