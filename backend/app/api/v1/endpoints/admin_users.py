...
from app.schemas.admin_user import AdminUserCreate, AdminUserOut, AdminUserUpdate, PaginatedUsers
...
@router.get(
    ...
    description="Lista paginada, filtrable por rol y estado. Rol requerido: admin.",
...
async def list_users(
    ...
    items, total = await admin_user_service.list_users(
        session, pagination, role=role, is_active=is_active
    )
...
async def update_user(
    ...
    user = await admin_user_service.update_user(
        session, admin, user_id, email=body.email, full_name=body.full_name, role=body.role
    )
    return AdminUserOut.model_validate(user)
